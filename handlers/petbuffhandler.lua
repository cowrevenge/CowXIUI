...
--
-- HXUI compatibility note: the original XIUI version hard-required
-- 'handlers.helpers' and 'libs.bufftable'. Those are XIUI-specific. In HXUI
-- the GetPlayerEntity / GetEntity globals are already populated by HXUI's
-- own bootstrap, and libs.bufftable is used only as a narrow fallback - so
-- we pcall both and gracefully degrade when they're absent.

require('common');
pcall(require, 'handlers.helpers');  -- sets GetPlayerEntity/GetEntity globals in XIUI; no-op in HXUI
local buffTable
do
    local ok, mod = pcall(require, 'libs.bufftable')
    if ok then buffTable = mod end  -- stays nil on HXUI; use-site checks this
end
...
local function tryApplyUnknownStatus(message, param, pkt, alreadyHandled)
    ...
    if (not effectId or effectId == 0 or effectId > 1000) and pkt and pkt.Type == 4 then
        -- Fallback: resolve spell-id -> buff-id via bufftable. On HXUI this
        -- lookup is skipped; the function just returns below when effectId
        -- stays nil. Most pet-ability buffs come through without needing it.
        if buffTable and buffTable.GetBuffIdBySpellId then
