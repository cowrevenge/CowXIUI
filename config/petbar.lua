...
local function DrawPetTypeVisualSettings(configKey, petTypeLabel)
    ...
    if components.CollapsingSection('Background##' .. configKey, false) then
        ...
        components.DrawPartySlider(typeSettings, 'Background Scale##' .. configKey, 'bgScale', 0.1, 3.0, '%.2f', nil, 1.0);
        imgui.ShowHelp('Scale of the background texture.');
        components.DrawPartySlider(typeSettings, 'Border Scale##' .. configKey, 'borderScale', 0.1, 3.0, '%.2f', nil, 1.0);
    ...
    if components.CollapsingSection('Ability Recasts##' .. configKey, false) then
        ...
        if typeSettings.showTimers then
            ...
            -- Recast display: name + timer + bar (the only mode).
            do
                imgui.Indent(10);
            ...
            -- Offsets only apply when not anchored (anchored mode flows inline).
            local isAnchoredFull = not typeSettings.iconsAbsolute;
            ...
            if configKey == 'petBarAvatar' then
                ...
                imgui.Separator();
                imgui.Spacing();
                components.DrawCheckbox('Astral Flow (2hr)', 'petBarSmnShowAstralFlow');
                imgui.ShowHelp('Show Astral Flow (Summoner 2-hour) in the bar.');
                ...
                -- Blood Pact favorites: clicking the Rage/Ward row in the bar
                -- fires the chosen favorite for the selected avatar.
                imgui.Spacing();
                imgui.Separator();
                imgui.Text('Click-to-Cast Favorites');
                imgui.ShowHelp('Pick a favorite Rage and Ward per avatar. Clicking the Blood Pact row in the pet bar casts it (Rage on target, Ward on self).');

                if gConfig.petBarSmnBloodPacts == nil then
                    gConfig.petBarSmnBloodPacts = T{};
                end

                -- Avatar selector (only avatars that have Blood Pacts).
                local bpAvatars = { 'Carbuncle', 'Ifrit', 'Shiva', 'Garuda', 'Titan', 'Ramuh', 'Leviathan', 'Fenrir', 'Diabolos' };
                local selAvatar = gConfig.petBarBpFavAvatar or bpAvatars[1];
                components.DrawComboBox('Avatar##bpFav', selAvatar, bpAvatars, function(v)
                    gConfig.petBarBpFavAvatar = v;
                end);
                selAvatar = gConfig.petBarBpFavAvatar or bpAvatars[1];

                if gConfig.petBarSmnBloodPacts[selAvatar] == nil then
                    gConfig.petBarSmnBloodPacts[selAvatar] = T{ rage = '', ward = '' };
                end
                local favs = gConfig.petBarSmnBloodPacts[selAvatar];
                local rageList, wardList = petData.GetBloodPactChoices(selAvatar);

                local curRage = (favs.rage ~= nil and favs.rage ~= '') and favs.rage or '(none)';
                components.DrawComboBox('Favorite Rage##bpFavRage', curRage, rageList, function(v)
                    gConfig.petBarSmnBloodPacts[selAvatar].rage = (v == '(none)') and '' or v;
                end);

                local curWard = (favs.ward ~= nil and favs.ward ~= '') and favs.ward or '(none)';
                components.DrawComboBox('Favorite Ward##bpFavWard', curWard, wardList, function(v)
                    gConfig.petBarSmnBloodPacts[selAvatar].ward = (v == '(none)') and '' or v;
                end);
            elseif configKey == 'petBarCharm' then
                imgui.Spacing();
                imgui.Separator();
                imgui.Spacing();
                components.DrawCheckbox('Familiar (2hr)', 'petBarBstShowFamiliar');
                imgui.ShowHelp('Show Familiar (Beastmaster 2-hour) in the bar.');
            ...
            elseif configKey == 'petBarAutomaton' then
                ...
                imgui.Separator();
                imgui.Spacing();
                components.DrawCheckbox('Overdrive (2hr)', 'petBarPupShowOverdrive');
                imgui.ShowHelp('Show Overdrive (Puppetmaster 2-hour) in the bar.');
            ...
            elseif configKey == 'petBarWyvern' then
                ...
                imgui.Separator();
                imgui.Spacing();
                components.DrawCheckbox('Spirit Surge (2hr)', 'petBarDrgShowSpiritSurge');
                imgui.ShowHelp('Show Spirit Surge (Dragoon 2-hour) in the bar.');
    ...
    if configKey == 'petBarCharm' then
        if components.CollapsingSection('Charm Indicator##charm', false) then
            ...
            imgui.ShowHelp('Show heart icon and the time remaining before the charmed pet breaks free.');
            ...
            imgui.ShowHelp('Font size for the charm time-remaining timer.');
...
local function DrawPetBarSettingsContent()
    ...
    components.DrawCheckbox('Clickable Abilities', 'petBarClickable');
    imgui.ShowHelp('Click an ability in the bar to use it. 2-hour abilities require a second click to confirm.');

    components.DrawCheckbox('Show Summon Row When Petless', 'petBarShowSummonRow');
    imgui.ShowHelp('When you have no pet, show a clickable Call Beast / Call Wyvern / Activate row.');

    components.DrawCheckbox('Show Pet Status Effects', 'petBarShowStatusEffects');
    imgui.ShowHelp('Show a text list of the pet\'s active buffs/debuffs below the bars. Buffs show remaining time.');
end
