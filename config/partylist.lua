...
local function DrawPartyTabContent(party, partyName)
    local layoutItems = { [0] = 'Horizontal', [1] = 'Compact Vertical', [2] = 'Super Compact' };
    local statusThemeItems = { [0] = 'HorizonXI', [1] = 'HorizonXI-R', [4] = 'Disabled' };
    ...
    if components.CollapsingSection('Scale & Spacing##party' .. partyName) then
        ...
        components.DrawPartyCheckbox(party, 'Alternating Member Colors', 'alternatingColors');
        imgui.ShowHelp('Tint every other party member with a subtle band so members are easier to tell apart.');
    ...
    if components.CollapsingSection('Text Settings##party' .. partyName) then
        ...
        components.DrawPartyCheckbox(party, 'Override Font', 'overrideFont');
        imgui.ShowHelp('Use a different font family and weight for this party list instead of the Global Text Settings.');

        if party.overrideFont then
            imgui.Indent(20);
            components.DrawComboBox('Font Family##party' .. partyName,
                party.fontFamily or 'Default',
                components.available_fonts,
                function(newValue)
                    party.fontFamily = newValue;
                    SaveSettingsOnly();
                    UpdateUserSettings();
                end);
            ...
            components.DrawComboBox('Font Weight##party' .. partyName,
                party.fontWeight or 'Normal',
                {'Normal', 'Bold'},
                function(newValue)
                    party.fontWeight = newValue;
                    SaveSettingsOnly();
                    UpdateUserSettings();
                end);

            components.DrawPartySlider(party, 'Outline Width', 'fontOutlineWidth', 0, 2);
...
function M.DrawSettings(state)
    ...
    components.DrawCheckbox('Shift to Drag', 'partyListShiftDrag');
    imgui.ShowHelp('When ON (default), the party windows only move while holding Shift. Prevents accidentally yanking the window when clicking on members.');
    components.DrawCheckbox('Show Target Bar', 'showPartyListTarget');
