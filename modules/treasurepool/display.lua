...
local bovinelooty = require('modules.treasurepool.bovinelooty');
...
function M.DrawWindow(settings)
    ...
    if selectedTab == 3 then
        -- BovineLooty tab: fixed-height content (button row + two list regions).
        local bvlRowH = fontSize + 6;
        local bvlListRows = 6;  -- visible rows per list
        totalContentHeight = (fontSize + 8)
            + (fontSize + 8) + (bvlListRows * bvlRowH)
            + (fontSize + 8) + (bvlListRows * bvlRowH)
            + (fontSize + 6);
    ...
    if imgui.Begin('TreasurePool', true, windowFlags) then
        ...
        if not drawList then
            if bovinelooty.HideTabContent then bovinelooty.HideTabContent(); end
            imgui.End();
            return;
        end
        ...
        if showTitle then
            ...
            -- Draw BovineLooty tab (tab 3)
            local bovineTabX = historyTabX + tabBtnWidth + btnSpacing;
            local bovineTabColors = (selectedTab == 3) and TAB_COLORS_SELECTED or TAB_COLORS_UNSELECTED;
            local bovineTabClicked = button.DrawPrim('tpTabBovine', bovineTabX, btnY, tabBtnWidth + fontSize * 2, btnHeight, {
                colors = bovineTabColors,
                tooltip = 'BovineLooty auto-lot/pass',
            });
            if bovineTabClicked then
                selectedTab = 3;
            end
            if data.tabBovineFont then
                data.tabBovineFont:set_font_height(fontSize);
                data.tabBovineFont:set_text('Looty');
                local bvTextW, bvTextH = data.tabBovineFont:get_text_size();
                data.tabBovineFont:set_position_x(bovineTabX + ((tabBtnWidth + fontSize * 2) - (bvTextW or 0)) / 2);
                data.tabBovineFont:set_position_y(btnY + (btnHeight - (bvTextH or fontSize)) / 2);
                data.tabBovineFont:set_visible(true);
                local bvTextColor = (selectedTab == 3) and 0xFFFFFFFF or 0xFFAAAAAA;
                if data.lastColors.tabBovine ~= bvTextColor then
                    data.tabBovineFont:set_font_color(bvTextColor);
                    data.lastColors.tabBovine = bvTextColor;
                end
            end
            ...
            if selectedTab == 1 then
                ...
                -- Pass All anchored to the right wall, just left of the minimize button.
                local passAllX = minimizeX - btnSpacing - textBtnWidth;
        ...
        -- Hide BovineLooty tab content whenever it isn't actively drawn this
        -- frame: either not the active tab, or the window is minimized. Without
        -- this, the Looty GDI fonts/buttons stay on screen (stuck) because
        -- nothing sets them invisible.
        if (selectedTab ~= 3 or isMinimized) and bovinelooty.HideTabContent then
            bovinelooty.HideTabContent();
        end
        if isMinimized then
            ...
            for slot = 0, data.MAX_POOL_SLOTS - 1 do
                ...
                button.HidePrim(string.format('tpAddLot%d', slot));
                button.HidePrim(string.format('tpAddPass%d', slot));
            end
            -- Hide history fonts
        ...
        for i, item in ipairs(poolItems) do
            ...
            -- 3b. BovineLooty [+] add-to-pass-list button.
            -- Drawn whenever the BovineLooty window is open, INDEPENDENT of the
            -- Lot/Pass button visibility (showButtons). This is deliberate: the
            -- pool may be collapsed with per-item L/P buttons disabled, but the
            -- user still needs to tag items for auto-pass while looty is open.
            -- Right-anchored to the same area the Lot button would occupy, so it
            -- sits consistently whether or not L/P are shown. Plain colored prim
            -- with a tooltip (no GDI font, so no init.lua font-lifecycle change).
            do
                -- Two square boxes per row: BLUE = add to Auto-Lot, RED = add to
                -- Auto-Pass. Shown whenever the BovineLooty window is open,
                -- independent of the Lot/Pass button visibility. Clicking toggles
                -- membership; lists are mutually exclusive (handled in the module).
                -- Preview rows are real mock IDs, so clicks DO register (the user
                -- confirmed add-for-real in preview to prove the flow).
                local lotBoxId  = string.format('tpAddLot%d', slot);
                local passBoxId = string.format('tpAddPass%d', slot);
                local boxH = fontSize + 4;
                local boxSpacing = 4;
                local boxY = textY - 1;
                local boxesVisible = isFontVisible(boxY, boxH);
                if bovinelooty.IsOpen() and boxesVisible and item.itemId ~= nil and item.itemId > 0 then
                    -- Anchor near the right edge, left of the timer text.
                    local boxTimerWidth = 0;
                    if showTimerText and data.timerFonts[slot] then
                        boxTimerWidth = data.timerFonts[slot]:get_text_size() or (fontSize * 3);
                    end
                    -- Rightmost edge for the box pair.
                    local rightX = startX + windowWidth - padding - itemPadding - boxTimerWidth - boxSpacing;
                    -- If L/P buttons also show, shift left past them (2 buttons).
                    if (isExpanded or gConfig.treasurePoolShowButtonsInCollapsed) then
                        local lpWidth = fontSize * 2.5;
                        rightX = rightX - (lpWidth + boxSpacing) * 2;
                    end
                    -- Pass (red) box on the right, Lot (blue) box to its left.
                    local passBoxX = rightX - boxH;
                    local lotBoxX  = passBoxX - boxSpacing - boxH;

                    local onLot  = bovinelooty.IsOnLotList(item.itemId);
                    local onPass = bovinelooty.IsOnPassList(item.itemId);

                    -- BLUE box -> Auto-Lot. Always blue; brighter when listed.
                    local lotColors = onLot and {
                        normal = 0xFF3d7aDD, hovered = 0xFF5a9aFF, pressed = 0xFF2d5aAA, border = 0xFF1a1a33,
                    } or button.COLORS_INFO;
                    local lotClicked = button.DrawPrim(lotBoxId, lotBoxX, boxY, boxH, boxH, {
                        colors = lotColors,
                        tooltip = onLot and 'On Auto-Lot list (click to remove)'
                                         or 'Add to Auto-Lot list',
                    });
                    if lotClicked then
                        if onLot then
                            bovinelooty.RemoveFromLotList(item.itemId);
                        else
                            bovinelooty.AddToLotList(item.itemId, item.itemName);
                        end
                    end

                    -- RED box -> Auto-Pass. Always red; brighter when listed.
                    local passColors = onPass and {
                        normal = 0xFFDD3d3d, hovered = 0xFFFF5a5a, pressed = 0xFFAA2d2d, border = 0xFF331a1a,
                    } or button.COLORS_NEGATIVE;
                    local passClicked = button.DrawPrim(passBoxId, passBoxX, boxY, boxH, boxH, {
                        colors = passColors,
                        tooltip = onPass and 'On Auto-Pass list (click to remove)'
                                          or 'Add to Auto-Pass list',
                    });
                    if passClicked then
                        if onPass then
                            bovinelooty.RemoveFromPassList(item.itemId);
                        else
                            bovinelooty.AddToPassList(item.itemId, item.itemName);
                        end
                    end
                else
                    button.HidePrim(lotBoxId);
                    button.HidePrim(passBoxId);
                end
            end
        ...
        elseif selectedTab == 2 then
            ...
            -- ============================================
            -- BOVINELOOTY TAB (tab 3): auto-lot/pass management
            -- ============================================
            -- Hide pool + history fonts/buttons so they don't bleed through.
            for slot = 0, data.MAX_POOL_SLOTS - 1 do
                if data.itemNameFonts[slot] then data.itemNameFonts[slot]:set_visible(false); end
                if data.timerFonts[slot] then data.timerFonts[slot]:set_visible(false); end
                if data.lotFonts[slot] then data.lotFonts[slot]:set_visible(false); end
            ...
            -- Render BovineLooty content into the pool window frame.
            if bovinelooty.DrawTabContent then
                bovinelooty.DrawTabContent(startX + padding, y, windowWidth - padding * 2, fontSize);
            end
        end
    else
        -- imgui collapsed/clipped the window (Begin returned false): hide the
        -- Looty GDI content so it doesn't stay stuck on screen.
        if bovinelooty.HideTabContent then bovinelooty.HideTabContent(); end
    end
    imgui.End();
end
...
function M.HideWindow()
    ...
    if data.tabPoolFont then data.tabPoolFont:set_visible(false); end
    if data.tabHistoryFont then data.tabHistoryFont:set_visible(false); end
    if data.tabBovineFont then data.tabBovineFont:set_visible(false); end

    -- Hide BovineLooty tab content (fonts + button prims).
    if bovinelooty.HideTabContent then bovinelooty.HideTabContent(); end
...
function M.Cleanup()
    ...
    for slot = 0, data.MAX_POOL_SLOTS - 1 do
        ...
        button.DestroyPrim(string.format('tpAddLot%d', slot));
        button.DestroyPrim(string.format('tpAddPass%d', slot));
...
-- BovineLooty tab accessors (tab 3).
function M.IsBovineLootyTabActive()
    return selectedTab == 3;
end

function M.SelectBovineLootyTab()
    selectedTab = 3;
end

function M.ToggleBovineLootyTab()
    if selectedTab == 3 then
        selectedTab = 1;  -- back to Pool
    else
        selectedTab = 3;
    end
    return selectedTab == 3;
