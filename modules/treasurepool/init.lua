...
M.lootyLocked = false;  -- When true, keep the window open on the Looty tab
...
function M.Initialize(settings)
    ...
    if gConfig then
        -- Preview persists across loads like every other module's Preview
        -- toggle (it only renders while the config menu is open). Default it
        -- to false on first run if unset, but do not force-clear on every load.
        if gConfig.treasurePoolPreview == nil then gConfig.treasurePoolPreview = false; end
    ...
    data.tabBovineFont = safeCreateFont(fontSettings);
    ...
    -- Provide font settings to BovineLooty so it can build its GDI window fonts.
    if bovinelooty.Configure then
        bovinelooty.Configure(settings);
    end
    ...
    data.allFonts = {data.headerFont, data.lotAllFont, data.passAllFont, data.toggleFont,
                     data.tabPoolFont, data.tabHistoryFont, data.tabBovineFont};
    for slot = 0, data.MAX_POOL_SLOTS - 1 do
...
function M.UpdateVisuals(settings)
    ...
    if data.tabBovineFont then
        data.tabBovineFont = FontManager.recreate(data.tabBovineFont, fontSettings);
    end
...
function M.DrawWindow(settings)
    ...
    -- BovineLooty automation tick (auto-lot/pass + dialog polling). Runs every
    -- frame regardless of window visibility so automation continues while the
    -- pool window is hidden/auto-hidden mid-fight.
    bovinelooty.Tick();

    if not M.visible then
        -- Module hidden entirely: ensure Looty GDI content is cleared too.
    ...
    -- Keep the window visible (overriding auto-hide) while the BovineLooty tab
    -- is selected, so the tab is reachable even when the pool is empty.
    local lootyTabActive = display.IsBovineLootyTabActive and display.IsBovineLootyTabActive();
    -- Lock keeps the window visible regardless of tab/content until toggled off.
    local lootyForce = lootyTabActive or M.lootyLocked;
    local showWindow;
    if autoHide then
        -- Auto-hide enabled: only show if pool has items (or preview/force show/looty tab)
        showWindow = (hasRealItems or data.previewEnabled or M.forceShow or lootyForce) and enabled;
    else
        -- Auto-hide disabled: show if pool has items OR has history
        showWindow = (hasRealItems or hasHistory or data.previewEnabled or M.forceShow or lootyForce) and enabled;
    end
...
function M.Cleanup()
    ...
    data.tabBovineFont = FontManager.destroy(data.tabBovineFont);
    ...
    -- Cleanup BovineLooty GDI fonts/background
    if bovinelooty.Cleanup then
        bovinelooty.Cleanup();
    end
...
-- ============================================
-- BovineLooty Interface (auto-pass)
-- ============================================

function M.BovineLootyToggleWindow()
    -- Toggle the "lock Looty open" state. When locked: switch to the Looty tab
    -- and keep the window visible (ignoring auto-hide) until unlocked.
    M.lootyLocked = not M.lootyLocked;
    if M.lootyLocked then
        display.SelectBovineLootyTab();
    end
    return M.lootyLocked;
end

function M.BovineLootyIsLocked()
    return M.lootyLocked == true;
end

function M.BovineLootyStart()
    bovinelooty.Start();
end

function M.BovineLootyStop()
    bovinelooty.Stop();
end

function M.BovineLootyToggleRun()
    return bovinelooty.Toggle();
end

function M.BovineLootyIsRunning()
    return bovinelooty.IsRunning();
end

function M.BovineLootyIsOpen()
    return display.IsBovineLootyTabActive();
end

function M.BovineLootyLoad()
    bovinelooty.RequestLoad();
end

function M.BovineLootySave()
    bovinelooty.RequestSave();
end

function M.BovineLootyAddLot(itemId)
    bovinelooty.AddToLotList(itemId, nil);
end

function M.BovineLootyAddPass(itemId)
    bovinelooty.AddToPassList(itemId, nil);
end

function M.BovineLootyRemove(itemId)
    -- Remove from whichever list it's on.
    bovinelooty.RemoveFromLotList(itemId);
    bovinelooty.RemoveFromPassList(itemId);
end
...
function M.SetPreview(enabled)
    ...
    -- When turning preview OFF, the mock items vanish but the GDI prims/fonts
    -- drawn for them don't hide themselves. Force a full window hide so nothing
    -- (boxes, bars, "Configuration Mode" text) is left stuck on screen. The
    -- next frame re-shows the window if there are real items.
    if not enabled then
        display.HideWindow();
    end
end
