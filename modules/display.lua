...
-- ============================================
-- Clickable Ability Hit-Testing
-- ============================================
-- XIUI draws abilities to the background draw-list (not as ImGui items), so
-- click detection is manual: each ability row reports its screen rect, and on
-- a left mouse-down we test the cursor against those rects and fire the matching
-- command via data.FireAbilityClick. Respects gConfig.petBarClickable (default
-- on). For 2-hour abilities, FireAbilityClick does the two-click arm/confirm.
local function handleAbilityClicks(clickRects)
    if gConfig.petBarClickable == false then return; end
    if not clickRects or #clickRects == 0 then return; end

    local mx, my = imgui.GetMousePos();
    if not mx then return; end

    for _, r in ipairs(clickRects) do
        if mx >= r.x1 and mx <= r.x2 and my >= r.y1 and my <= r.y2 then
            -- Resolve the command for hover feedback (nil = not clickable).
            local cmd, needsConfirm = data.ResolveAbilityCommand(r.name);
            if cmd then
                -- Manual tooltip near the cursor (these are draw-list rows, not
                -- ImGui items, so imgui.SetTooltip won't attach). Show the
                -- command, or a confirm prompt when a 2-hour is armed/arming.
                local tip;
                if needsConfirm and not data.IsAbilityArmed(r.name) then
                    tip = 'Click to arm: ' .. cmd;
                elseif needsConfirm and data.IsAbilityArmed(r.name) then
                    tip = 'Click again to CONFIRM: ' .. cmd;
                else
                    tip = cmd;
                end
                local dl = imgui.GetForegroundDrawList();
                if dl then
                    local tw = imgui.CalcTextSize(tip);
                    local tx, ty = mx + 14, my + 4;
                    dl:AddRectFilled({tx - 3, ty - 2}, {tx + tw + 3, ty + 16}, imgui.GetColorU32({0, 0, 0, 0.85}), 3);
                    dl:AddText({tx, ty}, imgui.GetColorU32({1, 1, 1, 1}), tip);
                end
                -- Fire on click.
                if imgui.IsMouseClicked(0) then
                    data.FireAbilityClick(r.name);
                end
            end
            break;
        end
...
function display.DrawWindow(settings)
    ...
    if petData == nil then
        ...
        -- No-pet summon row: when the player is a pet job with no pet out,
        -- show a clickable "[ Call Beast ]" / "[ Call Wyvern ]" / "[ Activate ]"
        -- row so they can summon from the bar. Skipped in preview mode and when
        -- disabled via gConfig.petBarShowSummonRow.
        local summon = nil;
        if gConfig.petBarShowSummonRow ~= false and not (showConfig[1] and gConfig.petBarPreview) then
            summon = data.GetNoPetSummonInfo();
        end

        if summon and data.nameText then
            data.HideBackground();
            local sFlags = data.getBaseWindowFlags();
            if gConfig.lockPositions then
                sFlags = bit.bor(sFlags, ImGuiWindowFlags_NoMove);
            end
            -- Reuse the saved pet bar position (same window name 'PetBar').
            if gConfig.petBarWindowPosX ~= nil and gConfig.petBarWindowPosY ~= nil then
                imgui.SetNextWindowPos({ gConfig.petBarWindowPosX, gConfig.petBarWindowPosY }, ImGuiCond_Once);
            end
            if imgui.Begin('PetBar', true, sFlags) then
                local wx, wy = imgui.GetWindowPos();
                local cx, cy = imgui.GetCursorScreenPos();

                -- Live recast: ready -> green/clickable, cooling -> dimmed with timer.
                local recast = data.GetSummonRecast(summon.timerId);
                local isReady = recast <= 0;
                local label;
                if isReady then
                    label = ('[ %s ]'):format(summon.label);
                else
                    label = ('[ %s %s ]'):format(summon.label, data.FormatTimer(recast));
                end
                local fontSize = gConfig.petBarVitalsFontSize or settings.vitals_font_settings.font_height;

                -- Hover/click handling against the label rect.
                local mx, my = imgui.GetMousePos();
                local tw = imgui.CalcTextSize(label);
                local hovered = mx and mx >= cx and mx <= cx + tw and my >= cy and my <= cy + fontSize + 2;
                local clickable = isReady and (gConfig.petBarClickable ~= false);

                data.nameText:set_font_height(fontSize);
                data.nameText:set_text(label);
                data.nameText:set_position_x(cx);
                data.nameText:set_position_y(cy);
                -- Green when ready (brighter on hover), grey while on cooldown.
                local col;
                if not isReady then
                    col = 0xFF888888;
                elseif hovered then
                ...
                data.nameText:set_font_color(col);
                data.nameText:set_visible(true);
                data.lastNameColor = nil;  -- force re-color next real pet

                imgui.Dummy({ tw, fontSize + 2 });

                if hovered and clickable then
                    local dl = imgui.GetForegroundDrawList();
                    if dl then
                        local tip = summon.cmd;
                        local ttw = imgui.CalcTextSize(tip);
                        dl:AddRectFilled({mx + 11, my + 2}, {mx + 17 + ttw, my + 20}, imgui.GetColorU32({0,0,0,0.85}), 3);
                        dl:AddText({mx + 14, my + 4}, imgui.GetColorU32({1,1,1,1}), tip);
                    end
                    if imgui.IsMouseClicked(0) then
            ...
            imgui.End();
            -- Hide the other fonts (we only used nameText).
            if data.distanceText then data.distanceText:set_visible(false); end
            if data.hpText then data.hpText:set_visible(false); end
            if data.mpText then data.mpText:set_visible(false); end
            if data.tpText then data.tpText:set_visible(false); end
            if data.bstTimerText then data.bstTimerText:set_visible(false); end
            if data.petStatusText then data.petStatusText:set_visible(false); end
            windowState.x = nil;
            windowState.y = nil;
    ...
    if imgui.Begin('PetBar', true, windowFlags) then
            ...
            if #timers > 0 then
                ...
                if displayStyle == 'full' then
                    ...
                    for i, timerInfo in ipairs(timers) do
                        ...
                        -- Record this row's clickable rect (icon + text width).
                        if timerInfo.name then
                            clickRects[#clickRects + 1] = {
                                name = timerInfo.name,
                                x1 = iconX, y1 = posY,
                                x2 = iconX + totalRowWidth, y2 = posY + contentHeight,
                            };
                            -- Armed 2-hour: overlay a bright confirm border.
                            if data.IsAbilityArmed(timerInfo.name) then
                                local armCol = imgui.GetColorU32({1.0, 0.85, 0.2, 1.0});
                                drawList:AddRect({iconX - 1, posY - 1}, {iconX + totalRowWidth + 1, posY + contentHeight + 1}, armCol, 3, nil, 2.0);
                            end
                        end
                    end
                    handleAbilityClicks(clickRects);
                    ...
                    for i, timerInfo in ipairs(timers) do
                        ...
                        if timerInfo.name then
                            clickRects[#clickRects + 1] = {
                                name = timerInfo.name,
                                x1 = startX, y1 = iconY,
                                x2 = startX + w, y2 = iconY + scaledIconSize,
                            };
                            -- Armed 2-hour: overlay a bright confirm border.
                            if data.IsAbilityArmed(timerInfo.name) then
                                local armCol = imgui.GetColorU32({1.0, 0.85, 0.2, 1.0});
                                drawList:AddRect({startX - 1, iconY - 1}, {startX + w + 1, iconY + scaledIconSize + 1}, armCol, 3, nil, 2.0);
                            end
                        end
                    end
                    handleAbilityClicks(clickRects);
        ...
        -- Pet Status Effects: simple comma-joined name list (no icons).
        -- Buffs that have a remaining time get a "(m:ss)" suffix; debuffs show
        -- name only. Hidden when there are no effects or the toggle is off.
        do
            local showStatus = gConfig.petBarShowStatusEffects ~= false;
            local effIds = petData.statusEffects;
            local effTimes = petData.statusEffectTimes or {};
            if showStatus and effIds and #effIds > 0 and data.petStatusText then
                local parts = {};
                for i = 1, math.min(#effIds, 12) do
                    local nm = data.GetStatusEffectName(effIds[i]);
                    local t = effTimes[i];
                    if t and t > 0 then
                        parts[#parts + 1] = string.format('%s (%s)', nm, data.FormatTimeMMSS(t));
                    else
                        parts[#parts + 1] = nm;
                    end
                end
                local statusStr = table.concat(parts, ', ');

                local statusFontSize = gConfig.petBarStatusFontSize or gConfig.petBarVitalsFontSize or settings.vitals_font_settings.font_height;
                local sx, sy = imgui.GetCursorScreenPos();
                local offsetX = gConfig.petBarStatusOffsetX or 0;
                local offsetY = gConfig.petBarStatusOffsetY or 0;

                data.petStatusText:set_font_height(statusFontSize);
                data.petStatusText:set_text(statusStr);
                data.petStatusText:set_position_x(sx + offsetX);
                data.petStatusText:set_position_y(sy + offsetY);

                local statusColor = colorConfig.statusTextColor or 0xFFCCFFCC;
                if data.lastPetStatusColor ~= statusColor then
                    data.petStatusText:set_font_color(statusColor);
                    data.lastPetStatusColor = statusColor;
                end
                data.petStatusText:set_visible(true);

                -- Reserve layout height so the window grows to fit the row.
                imgui.Dummy({ totalRowWidth, statusFontSize + 2 });
            elseif data.petStatusText then
                data.petStatusText:set_visible(false);
            end
        end
