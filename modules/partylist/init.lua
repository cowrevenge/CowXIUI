...
partyList.Cleanup = function()
    ...
    -- Destroy target bar's textured bg primitive (lazy-created on demand).
    if data.targetWindowPrim and data.targetWindowPrim.background then
        windowBg.destroy(data.targetWindowPrim.background);
        data.targetWindowPrim.background = nil;
        data.targetLoadedBg = nil;
    end
