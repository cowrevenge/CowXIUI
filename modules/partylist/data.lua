...
-- target bar visually matches the party panel.
data.targetWindowPrim = { background = nil };
data.targetLoadedBg   = nil;  -- last theme name applied (for setTheme on change)
...
-- Built-in defaults for Layout 2 (Super Compact). Used when gConfig.layoutSuperCompact
-- is absent — lets the layout work the moment a party's `layout = 2` flag flips,
-- even before the settings file or config menu have the template wired in.
-- Widths here are the raw pre-multiplier base values; display.lua applies the
-- global PARTY_BAR_BASE_WIDTH_MULT (0.8) before scale. Two distinct width fields:
--   hpBarWidth / mpBarWidth — actual rendered BAR widths (narrow, retail-like).
--   entryWidth              — the BOX/panel width; the text row (name + HP value)
--                             spans this width so it can extend past the bar's
--                             right edge. Bars never grow past hpBarWidth.
local LAYOUT_SUPERCOMPACT_DEFAULTS = {
    hpBarWidth  = 135,
    mpBarWidth  = 80,
    tpBarWidth  = 0,    -- TP renders as text only in super compact, no bar
    barHeight   = 12,
    barSpacing  = 4,
    entryWidth  = 160,
};
...
function data.getLayoutTemplate(partyIndex)
    ...
    if party.layout == 2 then
        return gConfig.layoutSuperCompact or LAYOUT_SUPERCOMPACT_DEFAULTS;
    elseif party.layout == 1 then
...
function data.updatePartyConfigCache()
    ...
    for partyIndex = 1, 3 do
        ...
        cache.alternatingColors = party.alternatingColors ~= false; -- Default true
...
-- Returns the background gradient for the depleted portion of party HP/MP/TP/cast bars.
-- When the user has configured a color override it wins; otherwise we fall back to
-- solid black so the depleted portion contrasts against the modern theme panel
-- (which otherwise washes it out — the bar background and the navy panel are too
-- close in luminance and the missing HP/MP/TP becomes invisible). Target bar uses
-- its own inline override (#000813) and bypasses this helper.
local DEFAULT_PARTY_BAR_BG = {'#000000', '#000000'};
...
function data.GetMemberInformation(memIdx)
    if (showConfig[1] and gConfig.partyListPreview) then
        ...
        local previewBuffs = {
            [0] = {3, 13, 33, 43, 5, 6},            -- Player 1: Poison, Slow, Haste, Stoneskin, Blind, Silence (buffs + curable debuffs)
            [1] = nil,                              -- Player 2: none (skip)
