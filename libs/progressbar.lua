...
local progressbar = {
	...
	-- Evicted entries held here for one frame before being allowed to GC.
	-- Releasing a D3D texture in the SAME frame it was referenced by an
	-- already-submitted imgui draw command (the draw list stores only a raw
	-- pointer, which does NOT keep the cdata alive) corrupts the device state
	-- for every draw AFTER it that frame -- which is what blanked the job/buff
	-- icons whenever an enemy's HP bar pushed the gradient cache over its cap.
	-- By holding evicted entries until the next eviction cycle, the texture is
	-- only released after the frame that used it has been flushed/presented.
	pendingFreeGradients = {},

	-- Cached U32 colors to avoid per-frame conversions
	colorU32Cache = {},

	-- Cache management settings
	-- Raised from 50: a full alliance of HP/MP/TP bars with damage/heal
	-- interpolation generates many unique color-pair keys per frame, so the
	-- old cap of 50 was crossed the instant an enemy bar was added, forcing
	-- eviction EVERY frame. A larger cap keeps eviction rare; entries are tiny
	-- (1x100 px textures) so the memory cost is negligible.
	MAX_GRADIENT_CACHE_SIZE = 512,  -- Maximum number of gradient textures before eviction
	EVICTION_COUNT = 32,            -- Number of oldest entries to remove when cache is full
...
local function EvictOldestEntries()
	...
	-- Release the PREVIOUS cycle's evicted textures now. By this point at least
	-- one full frame has elapsed since they were last referenced, so the draw
	-- commands that used them have been flushed/presented and it is safe to let
	-- their cdata go out of scope (which triggers gc_safe_release). We never
	-- release a texture in the same frame it was drawn -- that mid-frame device
	-- churn is what blanked downstream icon draws.
	for i = #progressbar.pendingFreeGradients, 1, -1 do
		progressbar.pendingFreeGradients[i] = nil;
	end
	...
	for i = 1, toRemove do
		...
		if entry then
			...
			-- Hold the entry (and thus its texture cdata) alive until the NEXT
			-- eviction cycle so it isn't GC-released during the current frame.
			progressbar.pendingFreeGradients[#progressbar.pendingFreeGradients + 1] = entry;
...
progressbar.DrawBar = function(startPosition, endPosition, gradientStart, gradientEnd, rounding, cornerFlags, drawList)
	...
	-- ===== A/B TEST =====
	-- When true: draw bars as PLAIN COLORED RECTS (no texture / no
	-- AddImageRounded).
	local _BISECT_PLAIN_RECT = false;
	if _BISECT_PLAIN_RECT then
		local startR, startG, startB = hex2rgb(gradientStart);
		local col = imgui.GetColorU32({ startR / 255, startG / 255, startB / 255, 1.0 });
		drawList:AddRectFilled(startPosition, endPosition, col, rounding, cornerFlags or ImDrawCornerFlags_All);
		return;
	end
	-- ===== END A/B TEST =====
...
progressbar.DrawBookends = function(positionStartX, positionStartY, width, height, drawList)
	...
	local _BISECT_PLAIN_RECT_BK = false;
	if _BISECT_PLAIN_RECT_BK then
		return;
	end
...
progressbar.ProgressBar  = function(percentList, dimensions, options)
	-- ===== A/B TEST 4: bars ON, borders OFF. The earlier plain-rect test
	-- showed bar fills alone don't uniquely break it, so suspect the border
	-- AddRect calls. _BISECT_DRAW_BARS draws the bar fills; _BISECT_DRAW_BORDER
	-- draws the border AddRects. If icons SURVIVE with bars on + borders off,
	-- the border AddRect is the corruptor. =====
	local _BISECT_DRAW_BARS   = true;
	local _BISECT_DRAW_BORDER = true;
	-- back-compat: the bar/overlay draw sites use _BISECT_DRAW
	local _BISECT_DRAW = _BISECT_DRAW_BARS;
	-- ===== END A/B TEST 4 =====
	if options == nil then
		options = {};
	end
	...
	-- ===== A/B TEST 5: force the FOREGROUND draw list instead of the window
	-- draw list. If icons SURVIVE with this, the corruption is specifically
	-- drawing into GetWindowDrawList() while called from inside the (still
	-- open) EnemyList window -- i.e. a draw-list/window interaction, not the
	-- draw primitive itself. =====
	local _BISECT_FOREGROUND_DL = true;
	local drawList;
	if _BISECT_FOREGROUND_DL and imgui.GetForegroundDrawList then
		drawList = options.drawList or imgui.GetForegroundDrawList();
	else
		drawList = options.drawList or imgui.GetWindowDrawList();
	end
	-- ===== END A/B TEST 5 =====
	...
	if options.overlayBar and _BISECT_DRAW then
	...
	if options.enhancedBorder and _BISECT_DRAW_BORDER then
	...
	local innerBorderThickness = gConfig.barBorderThickness or 2;
	if innerBorderThickness > 0 and _BISECT_DRAW_BORDER then
		local innerOffset = innerBorderThickness / 2;
		drawList:AddRect(
...
function progressbar.Cleanup()
	...
	progressbar.pendingFreeGradients = {};
