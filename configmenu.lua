...
config.DrawWindow = function(us)
    ...
    if(showConfig[1] and imgui.Begin(("HXUI Config"):fmt(addon.version), showConfig, bit.bor(ImGuiWindowFlags_NoSavedSettings))) then
        ...
        if (imgui.CollapsingHeader("General")) then
            imgui.BeginChild("GeneralSettings", { 0, 250 }, true);
            ...
            if (imgui.Checkbox('Modern Theme', { gConfig.modernTheme })) then
                gConfig.modernTheme = not gConfig.modernTheme;
                UpdateSettings();
            end
            imgui.ShowHelp('Unchecked: UI panels use the party-style textured background. Checked: clean dark-blue "cure window" look. Applies globally.');
        ...
        if (imgui.CollapsingHeader("Party List")) then
            ...
            if (imgui.Checkbox('Show Target Window', { gConfig.showPartyListTarget })) then
                gConfig.showPartyListTarget = not gConfig.showPartyListTarget;
                UpdateSettings();
            end
            if (imgui.Checkbox('Stacked Bars', { gConfig.stackedBars })) then
                gConfig.stackedBars = not gConfig.stackedBars;
                UpdateSettings();
            end
            if (imgui.Checkbox('Compact', { gConfig.partyListSuperCompactOverride })) then
                gConfig.partyListSuperCompactOverride = not gConfig.partyListSuperCompactOverride;
                UpdateSettings();
            end
            do
                local compactMode = gConfig.partyListSuperCompactMode or 1;
                local forceMode1  = gConfig.partyListSuperCompactOverride == true;
                if (imgui.RadioButton('Mode 1', forceMode1 or compactMode == 1)) then
                    gConfig.partyListSuperCompactMode = 1;
                    UpdateSettings();
                end
                imgui.SameLine();
                if (forceMode1) then
                ...
                if (imgui.RadioButton('Mode 2', not forceMode1 and compactMode == 2)) then
                    if (not forceMode1) then
                        gConfig.partyListSuperCompactMode = 2;
                        UpdateSettings();
                    end
                end
                if (forceMode1) then
            ...
            local masterScale = { gConfig.partyListMasterScale or 1 };
            if (imgui.SliderFloat('Master Scale', masterScale, 0.1, 3.0, '%.2f')) then
                gConfig.partyListMasterScale = masterScale[1];
                UpdateSettings();
            end

            local globalJobIconScale = { gConfig.partyListGlobalJobIconScale or 1 };
            if (imgui.SliderFloat('Global Job Icon Scale', globalJobIconScale, 0.1, 3.0, '%.2f')) then
                gConfig.partyListGlobalJobIconScale = globalJobIconScale[1];
                UpdateSettings();
            end

            local statusIconScale = { gConfig.partyListStatusIconScale or 1 };
            if (imgui.SliderFloat('Status Icon Scale', statusIconScale, 0.1, 3.0, '%.2f')) then
                gConfig.partyListStatusIconScale = statusIconScale[1];
                UpdateSettings();
            end

            if (imgui.Checkbox('Show Cure Buttons', { gConfig.partyListCureButtons })) then
                gConfig.partyListCureButtons = not gConfig.partyListCureButtons;
                UpdateSettings();
            end
            local cureButtonScale = { gConfig.partyListCureButtonScale or 1 };
            if (imgui.SliderFloat('Cure Button Scale', cureButtonScale, 0.1, 3.0, '%.2f')) then
                gConfig.partyListCureButtonScale = cureButtonScale[1];
                UpdateSettings();
            end
            imgui.EndChild();
            ...
            if true then
                ...
                if (imgui.SliderFloat('Party Job Icon Scale', jobIconScale, 0.1, 3.0, '%.1f')) then
        ...
        if (imgui.CollapsingHeader("Cast Cost")) then
            imgui.BeginChild("CastCostSettings", { 0, 90 }, true);
            if (imgui.Checkbox('Enabled', { gConfig.showCastCost })) then
                gConfig.showCastCost = not gConfig.showCastCost;
                UpdateSettings();
            end
            imgui.ShowHelp('Shows a floating info window about the currently-highlighted spell/ability/mount in the game\'s selection menus (MP cost, recast delay, TP for weapon skills). Anchored above the Target preview.');
            imgui.TextColored({0.6, 0.6, 0.6, 1.0}, 'Use the "Modern Theme" toggle under General to switch look.');
            imgui.EndChild();
        end
        if (imgui.CollapsingHeader("Treasure Pool")) then
            imgui.BeginChild("TreasurePoolSettings", { 0, 90 }, true);
            if (imgui.Checkbox('Enabled', { gConfig.treasurePoolEnabled })) then
                gConfig.treasurePoolEnabled = not gConfig.treasurePoolEnabled;
                UpdateSettings();
            end
            imgui.ShowHelp('Shows a floating list of current treasure pool items with countdown timers and highest lot.');
            imgui.Text('Window auto-hides when pool is empty.');
            imgui.TextColored({0.6, 0.6, 0.6, 1.0}, 'Drag to reposition. Lock HUD Position to prevent accidental moves.');
            imgui.EndChild();
        end
        if (imgui.CollapsingHeader("Pet HUD")) then
            imgui.BeginChild("BstHudSettings", { 0, 320 }, true);
            if (imgui.Checkbox('Enabled', { gConfig.showBstHud })) then
                gConfig.showBstHud = not gConfig.showBstHud;
                UpdateSettings();
            end
            imgui.ShowHelp('Floating pet HUD for BST / DRG / SMN / PUP. Shows pet HP, TP, MP, charges, ready moves, and the pet\'s current target.');

            if (imgui.Checkbox('Enabled for Dragoons', { gConfig.bsthudShowOnDrg })) then
                gConfig.bsthudShowOnDrg = not gConfig.bsthudShowOnDrg;
                UpdateSettings();
            end
            imgui.ShowHelp('Uncheck to suppress the HUD entirely while your main job is DRG. BST and SMN are unaffected.');

            imgui.Separator();

            if (imgui.Checkbox('Show Pet Portrait', { gConfig.bsthudShowImage })) then
                gConfig.bsthudShowImage = not gConfig.bsthudShowImage;
                UpdateSettings();
            end
            if (imgui.Checkbox('Show HP / TP Bars', { gConfig.bsthudShowBars })) then
                gConfig.bsthudShowBars = not gConfig.bsthudShowBars;
                UpdateSettings();
            end
            if (imgui.Checkbox('Show Ready Moves', { gConfig.bsthudShowMoves })) then
                gConfig.bsthudShowMoves = not gConfig.bsthudShowMoves;
                UpdateSettings();
            end
            imgui.ShowHelp('Beastmaster only. Lists the pet\'s known monster abilities with charge cost and damage type.');
            if (imgui.Checkbox('Show Damage Type Icons', { gConfig.bsthudShowIcons })) then
                gConfig.bsthudShowIcons = not gConfig.bsthudShowIcons;
                UpdateSettings();
            end
            if (imgui.Checkbox('Show Pet Target Panel', { gConfig.bsthudShowTarget })) then
                gConfig.bsthudShowTarget = not gConfig.bsthudShowTarget;
                UpdateSettings();
            end
            imgui.ShowHelp('Adds a small panel below the HUD showing what the pet is currently engaged with.');

            -- Use the `~= false` idiom so a nil (never-configured) default
            -- shows the checkbox as ON, matching the gate in bsthud.lua.
            do
                local isClickable = gConfig.bsthudClickable ~= false;
                if (imgui.Checkbox('Enable Clickable Pet HUD', { isClickable })) then
                    gConfig.bsthudClickable = not isClickable;
                    UpdateSettings();
                end
                imgui.ShowHelp('Click ability rows (Steady Wing, Spirit Link, Reward, etc.) to fire /ja. Click ready moves to fire /pet on your current target. Hover shows the exact command.');
            end

            imgui.Separator();

            local imageSize = { gConfig.bsthudImageSize };
            if (imgui.SliderInt('Portrait Size', imageSize, 50, 200)) then
                gConfig.bsthudImageSize = imageSize[1];
                UpdateSettings();
            end
            local bgAlpha = { gConfig.bsthudBgAlpha };
            if (imgui.SliderFloat('Background Alpha', bgAlpha, 0.0, 1.0, '%.2f')) then
                gConfig.bsthudBgAlpha = bgAlpha[1];
                UpdateSettings();
            end

            imgui.TextColored({0.6, 0.6, 0.6, 1.0}, 'Drag the window to reposition. /bsthud help for power-user commands.');
            imgui.EndChild();
            -- SMN Blood Pacts: nested collapsible inside the Pet HUD
            -- section. Rendered as its own BeginChild so the list is
            -- scrollable without inflating the parent. One row per avatar
            -- (spirits intentionally excluded - they're autonomous, no
            -- manual BPs). Each row gets a free-text Rage and Ward input;
            -- empty = that row on the pet HUD won't be clickable when
            -- that avatar is out. Initialized lazily on first open so
            -- saved configs with no smnBloodPacts key just get defaults.
            if (imgui.CollapsingHeader("SMN Blood Pacts")) then
                imgui.BeginChild("SmnBloodPactSettings", { 0, 340 }, true);
                imgui.TextColored({0.6, 0.6, 0.6, 1.0},
                    'Favorite BP per avatar. Clicking a Blood Pact row on the pet HUD fires the BP configured for whatever avatar is currently summoned.');
                imgui.Separator();

                if gConfig.smnBloodPacts == nil then
                    gConfig.smnBloodPacts = {};
                end

                -- SMN avatars only - no spirits. Order matches XIUI's
                -- data.avatarList for familiarity.
                local avatars = {
                    'Carbuncle', 'Ifrit', 'Shiva', 'Garuda', 'Titan', 'Ramuh',
                    'Leviathan', 'Fenrir', 'Diabolos', 'Atomos', 'Odin',
                    'Alexander', 'Cait Sith', 'Siren',
                };

                for _, avatarName in ipairs(avatars) do
                    if gConfig.smnBloodPacts[avatarName] == nil then
                        gConfig.smnBloodPacts[avatarName] = { rage = '', ward = '' };
                    end
                    local entry = gConfig.smnBloodPacts[avatarName];

                    imgui.Text(avatarName);
                    imgui.PushItemWidth(140);

                    local rageBuf = { entry.rage or '' };
                    if (imgui.InputText('Rage##bp_' .. avatarName, rageBuf, 64)) then
                        entry.rage = rageBuf[1];
                        UpdateSettings();
                    end
                    imgui.SameLine();
                    local wardBuf = { entry.ward or '' };
                    if (imgui.InputText('Ward##bp_' .. avatarName, wardBuf, 64)) then
                        entry.ward = wardBuf[1];
                        UpdateSettings();
                    end
